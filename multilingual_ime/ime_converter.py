...
class ChineseIMEConverter(IMEConverter):
    ...
    def get_candidates(self, key_stroke_query: str) -> list[CandidateWord]:
        ...
        candidates = [candidate for candidate in candidates if candidate["distance"] == min_distance]  # filter out candidates with distance greater than min_distance
...
class EnglishIMEConverter(IMEConverter):
    ...
    def get_candidates(self, key_stroke_query: str) -> list[CandidateWord]:
        ...
        min_distance = min([candidate["distance"] for candidate in candidates])
        candidates = [candidate for candidate in candidates if candidate["distance"] == min_distance]  # filter out candidates with distance greater than min_distance
        assert len(candidates) > 0, f"No candidate found for {key_stroke_query_lower_case}"
